...
public class BuyCar {
    ...
    double currentPercent = percentLossByMonth;
    double totalMoney = 0;
    int monthCounter = 0;
    
    while ((totalMoney + oldPrice) < newPrice){
    totalMoney += savingPerMonth;
    newPrice = decrease(newPrice, currentPercent);
    oldPrice = decrease(oldPrice, currentPercent);
    if(monthCounter % 2 == 0) { currentPercent += 0.5; }
    monthCounter++;
    }
		return new int[] { monthCounter, System.Convert.ToInt32((totalMoney + oldPrice) - newPrice) };
	}
  
  public static double decrease(double val, double percent) {
